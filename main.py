...
@app.get("/migrate_aws_to_azure_form", response_class=HTMLResponse)
...
async def aws_to_azure(
    ...
    try:
        aws_con = pyodbc.connect(
            "DRIVER={ODBC Driver 17 for SQL Server};PORT=%s;SERVER=%s;UID=%s;PWD=%s;Encrypt=yes;TrustServerCertificate=yes;Connection Timeout=30"
            % (aws_port, aws_server, aws_username, aws_password)
        )
        aws_cursor = aws_con.cursor()
        AZURE_SQL_CONNECTIONSTRING = (
            "Driver={ODBC Driver 17 for SQL Server};Server=tcp:%s.database.windows.net,1433;Database=%s;Encrypt=yes;TrustServerCertificate=no;Connection Timeout=30"
...
############### Migrate Azure to AWS ###############

@app.get("/migrate_azure_to_aws_form", response_class=HTMLResponse)
async def azure_to_aws_form():
    ...
    <body>
        ...
        <form action="/migrate_azure_to_aws" method="post" enctype="multipart/form-data">
...
############### Azure Import General ###############

@app.get("/Azure/Import/General", response_class=HTMLResponse)
...
<body>
    ...
    <form action="/Azure/Export/Structure" method="post" enctype="multipart/form-data">
...
@app.post("/Azure/Export/Structure/")
async def azure_get_tables(request: Request):
...
async def aws_schema_form():
    ...
    <body>
        <h1>Upload Schema and Data Files</h1>
        <form action="/AWS/Export/Structure" method="post" enctype="multipart/form-data">
            <label for="port">Port:</label><br>
            <input type="text" id="port" name="port" required><br><br>
...
@app.post("/AWS/Export/Structure")
async def aws_get_tables(
    request: Request,
    port: str  = Form(...),
    server: str  = Form(...),
    username: str  = Form(...),
...
############### Azure Export Data ###############
@app.post("/Azure/Export/Data")
async def azure_get_data(
    request: Request,
    server_name: str = Form(...),
    database_name: str = Form(...),
):
...
async def aws_schema_form():
    ...
    <body>
        <h1>Upload Schema and Data Files</h1>
        <form action="/AWS/Export/Data" method="post" enctype="multipart/form-data">
            <label for="port">Port:</label><br>
            <input type="text" id="port" name="port" required><br><br>
...
@app.post("/AWS/Export/Data")
async def aws_get_data(
    request: Request,
    port: str  = Form(...),
    server: str  = Form(...),
    username: str  = Form(...),
